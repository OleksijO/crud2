...
public class TechInfoBean implements Serializable {
    ...
    @PostConstruct
    public void init() {
        if (sessionBean.isLogged()&&(!InfoHolder.isLogged(sessionBean.getUserId()))){
           sessionBean.setUserId(-1);
            FacesContext.getCurrentInstance().addMessage(null,
                    new FacesMessage(FacesMessage.SEVERITY_INFO, "You session has expired. Please, log in again. ", null));
            FacesContext.getCurrentInstance().getPartialViewContext().getRenderIds().add("messages");
            FacesContext.getCurrentInstance().getPartialViewContext().getRenderIds().add("login-form");
        }
